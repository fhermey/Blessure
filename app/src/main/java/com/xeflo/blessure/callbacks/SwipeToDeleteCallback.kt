...
abstract class SwipeToDeleteCallback(context: Context) : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
    ...
    override fun onChildDraw(
        ...
        if ( dX.toInt().absoluteValue > 164) {
            ...
            deleteIcon?.setTint(Color.parseColor("#E01F3D"))
            deleteIcon?.draw(c)
        }

        super.onChildDraw(c, recyclerView, viewHolder, max(dX, -164f), dY, actionState, isCurrentlyActive)
    }
